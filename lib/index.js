 ...
 */
exports.npm = function(template, paths, saveTo = null, log = false, dependencyTypes=null) {
    return packageJsonMerge(template, paths, saveTo, log, "npm", dependencyTypes);
};
 ...
 * @throws {LogicalException} if a inputs provided cannot be logically processed according to defined behaviour.
 */
exports.yarn = function(template, paths, saveTo = null, log = false, dependencyTypes=null) {
    return packageJsonMerge(template, paths, saveTo, log, "yarn", dependencyTypes);
}
 ...
 * @private
 */
function packageJsonMerge(template, paths, saveTo, log, packageSpec, dependencyTypes=null) {
    // Inspect input.
    // template (and log)
    if (log) {
        log = console.log;
    ...
    npmValidate(template, packageSpec, log, dependencyTypes);
    // paths
    if (!_.isArray(paths)) {
    ...
    for (let filePath of paths) {
        ...
        npmValidate(pkg, packageSpec, log, dependencyTypes);
        packages.push(pkg);
    }
 ...
 * @private
 */
function npmValidate(pkg, pkgSpec, log, dependencyTypes=null) {
    let results = {
        valid: true
    };
